            ...
            <div class="col-12 mb-4">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div>
                        ...
                        <a href="{{ route('admin.sales.pdf', $sale->id_penjualan) }}" class="btn btn-primary"
                            target="_blank">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" value="{{ $sale->store->nama_toko }}"
                                            readonly>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="text" class="form-control" value="{{ $sale->store->nama_pemilik }}"
                                            readonly>
                                    </div>
                                </div>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            <tr>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        @if ($sale->product->foto_produk)
                                                            <img src="{{ asset('storage/' . $sale->product->foto_produk) }}"
                                                                class="rounded me-2" alt="Foto Produk"
                                                ...
                                                <td>Rp {{ number_format($sale->jumlah * $sale->harga_jual, 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        </tbody>
                                        <tfoot class="table-light">
                                            <tr>
                                                ...
                                                <td class="fw-bold">Rp
                                                    {{ number_format($sale->jumlah * $sale->harga_jual, 0, ',', '.') }}
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                ...
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body">
                        <div class="alert alert-info">
                            ...
                            <strong>Perhatian:</strong> Verifikasi penjualan akan mengurangi stok produk sebanyak
                            {{ $sale->jumlah }} unit. Penolakan penjualan tidak akan mempengaruhi stok.
                        </div>
                        ...
                        <form action="{{ route('admin.sales.update_status', $sale->id_penjualan) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status"
                                    ...
                                    <option value="pending" {{ $sale->status == 'pending' ? 'selected' : '' }}>Menunggu
                                        Verifikasi</option>
                                    <option value="verified" {{ $sale->status == 'verified' ? 'selected' : '' }}>
                                        Terverifikasi</option>
                                    <option value="rejected" {{ $sale->status == 'rejected' ? 'selected' : '' }}>Ditolak
                                    </option>
                                </select>
                                @error('status')
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('catatan_admin') is-invalid @enderror" id="catatan_admin" name="catatan_admin"
                                    rows="3">{{ old('catatan_admin', $sale->catatan_admin) }}</textarea>
                                @error('catatan_admin')
                                    <div class="invalid-feedback">
                                        {{ $message }}
                                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateAlertMessage(status) {
                ...
                if (status === 'verified') {
                    message =
                        '<i class="fas fa-info-circle me-2"></i><strong>Perhatian:</strong> Verifikasi penjualan akan mengurangi stok produk sebanyak {{ $sale->jumlah }} unit.';
                    alertInfo.className = 'alert alert-info';
                } else if (status === 'rejected') {
                    message =
                        '<i class="fas fa-exclamation-triangle me-2"></i><strong>Perhatian:</strong> Penolakan penjualan tidak akan mempengaruhi stok produk.';
                    alertInfo.className = 'alert alert-warning';
                } else {
                    message =
                        '<i class="fas fa-info-circle me-2"></i><strong>Perhatian:</strong> Status menunggu tidak akan mempengaruhi stok produk.';
                    alertInfo.className = 'alert alert-info';
                }
            ...
            // Initial update
            updateAlertMessage(statusSelect.value);

            // Update on change
            statusSelect.addEventListener('change', function() {
                updateAlertMessage(this.value);
            });
        });
    </script>
@endsection
